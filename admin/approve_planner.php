        ...
        case 'approve_planner':
            if (!empty($user_id)) {
                ...
                if ($stmt->execute()) {
                    ...
                    $success_message = 'Planner approved successfully! They can now login and access the system.';
                } else {
...
$query = "SELECT u.*, p.company_name, p.specialization, p.experience_years, p.hourly_rate, p.approval_status
          ...
          WHERE u.status = 'pending' AND u.user_type = 'planner'
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="stats-card">
                            <h4><?php echo count(array_filter($all_planners, function($p) { return $p['status'] === 'pending'; })); ?></h4>
                            <p class="mb-0">Pending Approval</p>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="stats-card">
                            <h4><?php echo count(array_filter($all_planners, function($p) { return $p['status'] === 'active'; })); ?></h4>
                            <p class="mb-0">Approved Planners</p>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="stats-card">
                            <h4><?php echo count(array_filter($all_planners, function($p) { return $p['status'] === 'rejected'; })); ?></h4>
                                    ...
                                    <?php foreach ($all_planners as $planner): ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                ?>
                                                <span class="badge <?php echo $status_class; ?>"><?php echo ucfirst($planner['status']); ?></span>
                                            </td>
                                            <td><?php echo date('M j, Y', strtotime($planner['created_at'])); ?></td>
                                            <td>
                                                <?php if ($planner['status'] === 'pending'): ?>
